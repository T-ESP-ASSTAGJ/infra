...
grep -vE '\s+vault(\.platform)?\.local' "$HOSTS_FILE" > "$TMP_HOSTS"
grep -vE '\s+pki(\.platform)?\.local' "$HOSTS_FILE" > "$TMP_HOSTS"

# Append new entry
echo -e "$RANDOM_IP pki.platform.local vault.platform.local monitoring.platform.local bytebase-ui.platform.local" >> "$TMP_HOSTS"
